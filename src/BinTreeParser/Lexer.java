...
public class Lexer {
	...
	public Lexer(InputStream in) throws java.io.IOException {
		...
		Pattern tokenPattern = Pattern.compile("leaf|branch|[0-9]+|\\(|\\)|,|\\s+");
		...
		while (m.find()) {
			...
			if (m.group().equals("leaf")) 
				tokens.add(new Token(TokenType.Leaf));
			else if (m.group().equals("branch"))
				tokens.add(new Token(TokenType.Branch));
